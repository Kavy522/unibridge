...
export default function UniversityHodsPage() {
  ...
  const assign = useMutation({
    mutationFn: (v: { facultyId: string; batchId: string }) => universityApi.assignScope(v.facultyId, v.batchId),
    onSuccess: () => { toast.success('Batch assigned'); refresh() },
    onError: (e) => toast.error(errorMessage(e)),
  })
    ...
    <PageShell title="HODs" subtitle={d?.activeSemester ? `Batch ownership for ${d.activeSemester.label}` : 'No active semester'}>
      <Card className="mb-5 p-4">
        ...
        <div className="flex gap-2">
          <Select value={promotePick} onChange={(e) => setPromotePick(e.target.value)} placeholder="Select faculty" className="max-w-xs"
            options={(d?.facultyOptions ?? []).map((f) => ({ value: f.id, label: `${f.name} (${f.employeeId} · ${f.department})` }))} />
      ...
      {q.isLoading ? (
        ...
        <div className="space-y-4">
          {d?.hods.map((h) => {
            const freeBatches = (d?.batches ?? []).filter((b) => !b.ownedBy)
            return (
              <Card key={h.id} className="p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-0.5 text-xs text-text-muted">{h.employeeId} · {h.department} · {h.email}</div>
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      {h.scopes.length === 0 ? (
                        <span className="text-xs text-text-muted">No batches assigned</span>
                      ) : (
                        h.scopes.map((s) => (
                  ...
                  <div className="flex items-center gap-2">
                    <Select value="" placeholder={freeBatches.length ? 'Assign batch…' : 'No free batches'} className="w-44"
                      onChange={(e) => e.target.value && assign.mutate({ facultyId: h.id, batchId: e.target.value })}
                      options={freeBatches.map((b) => ({ value: b.id, label: `Batch ${b.code} (${b.yearLevel})` }))} />
