...
export function StudentProfileModal({
  ...
  const detail = useQuery({
    queryKey: ['hod', 'student', enrollmentNo],
    queryFn: () => hodApi.students.get(enrollmentNo),
  })
  const history = useQuery({
    queryKey: ['hod', 'student', enrollmentNo, 'history'],
    queryFn: () => hodApi.students.history(enrollmentNo),
  })
              ...
              <ol className="relative space-y-4 border-l-2 border-border pl-5">
                {history.data.journey.map((j, i) => (
                  <li key={i} className="relative">
                    <span className="absolute -left-[27px] top-1 h-3 w-3 rounded-full border-2 border-primary bg-surface" />
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-sm font-semibold text-text-primary">
                        Semester {j.semesterNumber}
                      </span>
                      <Badge tone="primary">{j.yearLevel}</Badge>
                      <span className="text-xs text-text-muted">{j.academicYear}</span>
                    </div>
                    <div className="mt-0.5 text-xs text-text-secondary">
                      Batch {j.batchCode} · Roll {j.rollNo}
                    </div>
                  </li>
                ))}
              </ol>
